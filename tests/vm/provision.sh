...
# pacman abandons a transfer that moves slower than one byte for ten seconds,
# and takes the whole transaction down with it. Two runs on 2026-08-29 died
# there, both times because the host's link was busy rather than because a
# mirror was down. A slow install is a result. One that stops at 40% with half
# a root filesystem is not, and it says nothing about the installer.
log "Letting a slow mirror be slow"
