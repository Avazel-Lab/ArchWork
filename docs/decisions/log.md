...
- Before wiping a disk on shared hardware, prove the other systems boot without it. Observed on `hmlxdesktop01` on 2026-08-28: `efibootmgr -v` shows one `Windows Boot Manager` entry against `nvme0n1p4`, the ESP on the disk ArchWork is to take, and a second against `sdb3`, an ESP on the disk the Windows install itself lives on. The second is what survives the wipe, and it is ahead of the first in the boot order. Boot Windows from it once and confirm before `nvme0n1` is touched: an entry that exists is not the same as one that works.
