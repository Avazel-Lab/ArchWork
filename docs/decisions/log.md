...
- Not yet proven against the M4 exit criteria themselves: "an automated check asserts each timing and asserts that `systemd-inhibit --list` shows the expected lock during an inhibit window" is VM harness work, and needs the nested virtualisation this was written without access to. `docs/STATUS.yml` carries that as the current blocker, the same as D-027's hardware confirmation was until it had one.
