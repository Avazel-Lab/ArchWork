...
**Confirmed against `hmlxdesktop01` on 2026-09-05.** The Kubuntu install on the same physical machine, same firmware, same card, was read directly rather than reasoned about. All three units are `enabled` there. Its driver package generates `/etc/modprobe.d/nvidia-graphics-drivers-kms.conf` carrying `options nvidia NVreg_PreserveVideoMemoryAllocations=1`, and `/usr/lib/systemd/system/systemd-suspend.service.d/nvidia-suspend-nofreeze.conf` is the drop-in D-037 saw. That is the working configuration on the machine that resumes, next to its absence on the machine that does not.

**And it carries a third setting this decision had missed.** Ubuntu also sets `NVreg_TemporaryFilePath=/var`. That parameter is where the driver writes the whole of VRAM on the way into a sleep, and it defaults to `/tmp`, which systemd makes a tmpfs. The default therefore saves video memory into RAM in order to preserve it across a suspend that is about to stop refreshing RAM. On a 12 GB card that is either a failure or an enormous amount of RAM consumed at the worst possible moment, and enabling the units without it would have produced a fix that still did not work.

ArchWork points it at `/var/cache/archwork/nvidia`, which the packages role creates. `@var_cache` rather than `@` because a multi-gigabyte scratch file that exists only between a suspend and a resume is exactly what the rollback boundary is drawn to exclude, and D-018 and D-040 already put this kind of data there. Ubuntu's `/var` would work and would sit inside the boundary.

`archwork-health` checks it by resolving the parameter to a filesystem type and refusing `tmpfs` and `ramfs`. An unset parameter is read as `/tmp` rather than skipped, because the unconfigured case is the failing one.
...
**`hmlxdesktop01` reads `s2idle [deep]`, and that is the first thing to compare.** Read on 2026-09-05 from the Kubuntu install on the same board, an AMD 500 series chipset with two xHCI controllers, `1022:43ee` and `1022:149c`. The board offers both states and the kernel selects `deep`, so the machine that resumes cleanly is doing S3 rather than s2idle. If the Arch install selects `s2idle`, that is the difference, and s2idle on an AMD desktop board is a well-worn way to lose a USB controller across a resume.

It can be tested without a reboot and without touching anything this repository owns, which is why it comes first: `echo deep > /sys/power/mem_sleep` selects the state for the next suspend only. Nothing about that survives a reboot and nothing about it needs to.

**What to run on `hmlxdesktop02`, in this order.** Written out because the moment it matters is a moment when the machine has just misbehaved and the reboot that discards the evidence is one keystroke away.

```
# 1. The comparison. hmlxdesktop01 reads "s2idle [deep]".
cat /sys/power/mem_sleep

# 2. Is the failed boot still on disk. Only one entry means the journal is
#    volatile and the evidence from the earlier failures is already gone.
journalctl --list-boots

# 3. If it is there, capture it by ID rather than by -1: booting into Kubuntu
#    and back has moved it further down the list every time.
BOOT=<id from the list>
journalctl -b "$BOOT" -k > ~/usb-resume-fail.txt
journalctl -b "$BOOT" -k | grep -iE 'xhci|dpm_run_callback|PM: (suspend|resume|dpm)|-110'

# 4. For comparison with hmlxdesktop01: NVIDIA 595.84 open module, kernel
#    7.0.0-30-generic.
uname -r; cat /proc/driver/nvidia/version
```

`-110` is `ETIMEDOUT` out of `pci_pm_resume`, and it is what an xHCI controller that never came back looks like in the log. It is worth grepping for by itself.

If step 1 reads `[s2idle] deep`, stop and test that before reading anything else:
...
That selects the state for the next suspend only and does not survive a reboot, so it costs nothing and rules the largest candidate in or out in one attempt.

Only if that proves to be the cause does the question of `mem_sleep_default=deep` on the desktop kernel command line arise, and it is not a free change: `ansible/files/kernel-cmdline/desktop` is written once by `scripts/archwork-install.sh` and never reconciled afterwards, so an already-installed machine needs the file edited and its UKIs regenerated by hand. D-027 already flagged that gap and deliberately did not close it.
